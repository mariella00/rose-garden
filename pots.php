...
if(isset($_POST['go']))
{
    ...
    $query = "SELECT * FROM pots_products WHERE pots_name LIKE '%$search%'";
    $search_result = filterTable($query);
       $queryResult = mysqli_num_rows($search_result);
    }
 else {
    $query = "SELECT * FROM pots_products";
    $search_result = filterTable($query);
  ...
  <!--SEARCH----------------->
      <form method="post" action="pots.php">
            <div class="search-box">
                <input class="search-txt" type="text" name="search" placeholder="Search in pots">
...
<section class="product-list">
      ...
      <div class="product-container">
        <input type="hidden" name="id" value="<?php echo $row['pots_id']; ?>">
          <div class="card">

            <div class="image">
              <a href="pots_description.php?next=<?php echo $row['pots_id']; ?>">
                  <img src="<?php echo $row['pots_image1']; ?>">
              </a> 
            </div>
            <div>
            <a href="pots_description.php?next=<?php echo $row['pots_id']; ?>" style="text-decoration: none;  color: black;">
                <div class="title"><?php  if ($row['pots_category'] != "") {
                  echo $row['pots_category'];
                 }   ?></div>
            </a>
             <div class="text"><?php echo $row['pots_name']; ?></div>
            </div >

            <div class="cart-price">   
              <div class="price">
                <span>₱<?php echo $row['pots_price']; ?></span>
              </div>
